...
Because I don't really know how Unity works and I can't program in C# I thought, it would be good to first write a little prototype in p5.js. To begin with it, I took the example from Daniel Shiffman from The Coding Train in his Coding challenge #124: Flocking Simulation.  
...
As you can see, the balance for the flocking behavior is very fragile. Another issue is, that performance in p5.js for this kind of project is very limited and so i hoped, that Unity would do better.  
...
In the tutorial from Board To Bits Games Ben shows, how a flock can be realized in a modular way in Unity. Therefore, he uses scriptable objects. I heard of them before and was happy to give these a shot. The tutorial project is built in 2D, but I wanted to do it in 3D, so I had to do minor adjustments.  
...
When implementing the three rules of a flock every boid has to know about its neighbors. Therefore the flock class submits a list of nearby neighbor transforms to it. As a result we can see, that every boid is aware of its neighbors. In the picture the boids color turns more red the more of them it has.
...
In cohesion the boid tends to move to the middle of its neighbors.
...
And for alignment the boids tend to align their flight direction with that of their neighbors. When all rules are applied we already have some kind of flock.  
...
Now it is easy to just add new elements to the flock create different customized behaviors.  
...
I was excited about the thought of: "I'm gonna get so much creative code done and achieve real cool stuff!" Well, I did. It just wasn't as much as I thought it would be.
...
In the process of trial an error I noticed for myself that I in fact got faster at coding. So I think I achieved that goal.  
